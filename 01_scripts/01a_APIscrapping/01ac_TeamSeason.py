...
def main(
    ...
    if not fbref_cfg.get("enable_team_season", True):
        logger.info("Team season stats disabled via configuration")
        return
    ...
    for season_label, season_id in seasons.items():
        ...
        for league_key, league_id in leagues.items():
            ...
            logger.info(f"=== Team Season | {season_label} | {league_key} ({league_id}) ===")
            try:
            ...
            for stat_type in TEAM_SEASON_STATS:
                stat_camel = stat_to_camel(stat_type)
                logger.info(f"Fetching {stat_camel}...")
                df = safe_call(fbref.read_team_season_stats, stat_type=stat_type, opponent_stats=True)
                df = flatten_columns(df)
                if df.empty:
                    logger.warning(f"{stat_camel} returned no data")
                    continue
                output_path = RAW_DIR / season_label / league_key / "TeamSeason" / f"{stat_camel}.parquet"
